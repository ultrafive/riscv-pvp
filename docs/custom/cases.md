...
Verification cases should be defined following this template.

```yaml
vadd_vi: # test group name, use the instrution name for isa cases
  env: RVTEST_RV64UV # test env macro with init, defined in riscv_test.h
  head: | # 
    #include "exception.h"
  ...
  cases: # test cases by passing parameters to defined templates
  ...
  check: # check simulate result with python golden
